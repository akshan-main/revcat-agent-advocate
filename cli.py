...
def generate_application(ctx):
    ...
    with start_run(db, "generate-application", {}, config) as run_ctx:
        ...
        piece = ContentPiece(
            ...
            created_at=now_iso(),
            word_count=len(body_md.split()),
            citations_count=body_md.count("[Source]"),
        )
        record_content(db, piece)
...
def _generate_application_with_claude(config, doc_snippets, run_ctx):
    ...
    system = """You are writing a public application letter for the RevenueCat Agentic AI \
Developer & Growth Advocate role. This will be published at a URL and reviewed by \
RevenueCat's hiring council.

This is a proof-of-work demonstration. The letter was generated by the agent system itself, \
from ingested RevenueCat docs, with citations, logged to a tamper-evident ledger.

STRUCTURE (follow exactly):

## The Next 12 Months
- How agentic AI changes developer discovery and adoption of subscription tooling
- RevenueCat's API surface (26 MCP tools, Charts API, REST v2) enables programmatic adoption
- Frame around business outcomes: faster shipping cadence, honest measurement, reduced brand risk

## RevenueCat's Strategic Position
- LLM docs index with .md mirrors (append .md to any doc URL), agent-native documentation
- MCP Server: 26 tools via Streamable HTTP at https://mcp.revenuecat.ai/mcp
- Setup: `claude mcp add revenuecat -t http -- https://mcp.revenuecat.ai/mcp --header "Authorization: Bearer YOUR_API_KEY"`
- Charts API: MRR, ARR, churn, LTV, active subs. Production receipt snapshots. All times UTC.
- Based on public data, competitors don't currently offer MCP + LLM docs + Charts API
- Cite SPECIFIC doc pages. Use [Source](url) format.

## What This System Delivers
- /content/: technical posts with snapshot-verified citations
- /experiments/: growth experiments with hypotheses and measured results
- /feedback/: structured product feedback with repro steps and evidence
- /ledger/: hash-chained audit trail: hash = sha256(prev_hash + canonical_json)

## Business Outcomes
Frame everything as operational value:
- "Compresses cycle time" not "replaces people"
- "Reduces operational load" not "fires employees"
- "Honest measurement with uncertainty" not "10x better"
- "Reduced brand risk via governance" not "unbreakable"
Key outcomes: faster content shipping, measurable experiments, product improvements with evidence, governance safety

## Operating Model
- Weekly cadence with defined outputs
- Safety gates: DRY_RUN, ALLOW_WRITES, DEMO_MODE
- Verification: citations checked, docs hashed, code syntax validated
- Modes: CLI, chat, HTTP API, MCP server, autonomous scheduler
- Fully reproducible pipeline

## The Close
- "This is not a proposal. This is a running system with receipts."
- The letter itself is a ledger entry, verifiable at /ledger
- Quantify one demo run's output concisely
- Close: "Ready to start Week 1."

TONE: Measured, specific, evidence-driven. Write like a serious operator, not an excited engineer. \
No filler, no hedging, no "I believe" statements. No claims about replacing people or saving money. \
Focus on operational value and verifiable output. \
Write in markdown. Use real RevenueCat terminology from the provided docs."""
...
Agentic AI changes how developers discover, evaluate, and adopt subscription tooling. Autonomous agents are already managing in-app purchase lifecycles, optimizing paywalls, and querying analytics APIs. The developers who adopt RevenueCat next will do so because an agent recommended it.

RevenueCat is well-positioned for this shift. The API surface (26 MCP tools, Charts API, REST v2, webhooks) supports programmatic consumption. The MCP server and LLM docs index show RevenueCat is already investing in agent-friendly infrastructure.

## RevenueCat's Strategic Position

Three infrastructure decisions are worth highlighting:

**1. Agent-Native Documentation.** RevenueCat publishes an LLM-optimized docs index and provides `.md` mirrors for every documentation page (append `.md` to any doc URL). This reduces integration friction for any agent-powered workflow.

**2. MCP Server.** 26 tools via Streamable HTTP at `https://mcp.revenuecat.ai/mcp`: projects, apps, products, offerings, entitlements, customers, charts. Any MCP-compatible agent can connect in one command:
...
**3. Charts API.** Programmatic access to MRR, ARR, churn, LTV, active subscriptions, trials, refunds, and conversions. Data based on production receipt snapshots. All times UTC.

Based on public data, competitors (Adapty, Qonversion, Glassfy, Superwall) do not currently offer an MCP server or LLM-optimized doc mirrors.

## What This System Delivers

Browse the evidence:

- [/content/](/content/): Technical posts with snapshot-verified citations. Every factual claim links to a specific RevenueCat doc page.
- [/experiments/](/experiments/): Growth experiments with stated hypotheses, defined metrics, and measured results.
- [/feedback/](/feedback/): Structured product feedback with severity ratings, reproduction steps, and proposed fixes.
- [/ledger/](/ledger/): Hash-chained audit trail. `hash = sha256(prev_hash + canonical_json)`. Tamper with any entry and the chain breaks.

## Business Outcomes

**Compressed content cycle.** Content generation pipeline handles outline, draft, citation verification, and publish in one command. Every piece ships with verified sources.

**Honest measurement.** Growth experiments state hypotheses upfront and report results against defined metrics. No vanity numbers.

**Reduced brand risk.** Safety gates (`DRY_RUN`, `ALLOW_WRITES`) prevent unreviewed external actions. A 16-case red-team governance suite tests for prompt injection, PII disclosure, competitor bashing, and brand safety violations.

**Product improvements with evidence.** Feedback items include executable reproduction steps and golden snapshot diffs; product teams get actionable reports, not vague complaints.

## Operating Model

**Weekly cadence:**
- 2-4 content pieces with verified citations
- 1 growth experiment with hypothesis and measured results
- 3+ product feedback items with reproduction evidence
- 1 weekly report summarizing activity and outcomes

**Safety and verification:**
- `DRY_RUN=true`: drafts only, no external posting until approved
- `ALLOW_WRITES=false`: read-only API access by default
- Citation links validated, doc snapshots hashed at ingest, code syntax checked
- Full hash chain logged to tamper-evident ledger

**Modes of operation:**
- CLI pipeline (`revcat-advocate demo-run` for full end-to-end)
- Interactive chat (`revcat-advocate chat`)
- HTTP API and MCP server (other agents can connect to this system)
- Autonomous scheduler (`revcat-advocate auto --interval 6h`)

**Fully reproducible:** `revcat-advocate write-content --topic "Any Topic"` runs the same pipeline every time.

## The Close

This is not a proposal. This is a running system with receipts.

This application letter is itself a ledger entry. Check [/ledger/](/ledger/) to verify. The hash chain is intact. The citations are snapshot-verified.

One demo run produces content pieces, a growth experiment, structured feedback, a complete auditable site, and a verified ledger chain. The artifacts are at the links above.
